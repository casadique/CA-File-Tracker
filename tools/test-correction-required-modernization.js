...
assert.match(styles, /\.correction-required-client\s*\{[\s\S]*?width:\s*270px[\s\S]*?padding:\s*10px 12px !important/,
  "The Client column should provide sufficient usable width and balanced padding");
assert.match(styles, /\.correction-required-sn\s*\{[\s\S]*?width:\s*55px[\s\S]*?min-width:\s*55px[\s\S]*?max-width:\s*55px/,
...
assert.match(styles, /\.correction-required-modern-table\s*\{[\s\S]*?min-width:\s*1320px/,
  "The desktop table should scroll instead of compressing Client and Service columns");
assert.match(styles, /\.correction-required-client-line\s*\{[\s\S]*?display:\s*grid[\s\S]*?grid-template-columns:\s*auto minmax\(0, 1fr\)[\s\S]*?gap:\s*10px/,
  "The expand control and client details should share a wrapping grid inside the Client cell");
