...
assert.match(source, /`DAILY ACTIVITY REPORT- \$\{displayDate\(date\)\}`/);
assert.match(source, /doc\.setFontSize\(16\)/);
assert.match(source, /Generated: \$\{generatedAt\}  \|  Generated by: \$\{generatedBy\}  \|  Closing Balance: \$\{billedPdfMoney\(summary\.totalBalance\)\}/);
...
[
  ...
  "Received From", "Service Type", "Mode", "Ref No", "Amount",
].forEach((header) => assert.match(source, new RegExp(`header: "${header.replace(".", "\\.")}"`), `${header} PDF column must exist`));
assert.match(source, /\["SN", "Received From", "Service Type", "Mode", "Account", "Ref No", "Amount"\]/);
assert.match(source, /collection\.serviceType \|\| collection\.service_type \|\| collectionTypeLabel/);
