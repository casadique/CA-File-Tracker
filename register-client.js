(function registerModules() {
  ...
  async function addDscMovementForm(){
    ...
    const authorities=uniqueValues(["XtraTrust","Emudhra","Vsign"],optionResult.authorities,records.map((row)=>row.authority));
    ...
    showModal("Add DSC Movement",`<form id="dscMovementForm" class="register-form-grid dsc-movement-form-layout">
      ...
      <label data-movement-in hidden><span class="dsc-label-row">AUTHORITY<button type="button" data-dsc-add-option="authority" data-dsc-option-target="#movementAuthority" aria-label="Add Authority">+</button></span><select name="authority" id="movementAuthority"><option value="">Select Authority</option>${options(authorities)}</select></label>
      ...
      <label class="form-span-2 movement-span-all">REMARKS<textarea name="remarks" rows="2"></textarea></label>
      <div class="form-span-2 movement-span-all register-error" data-movement-error role="alert" hidden></div>
      <div class="modal-actions form-span-2 movement-span-all"><button type="button" class="secondary-button" data-close-register-modal>Cancel</button><button type="submit" class="primary-button">Save Movement</button></div>
    </form>`);
