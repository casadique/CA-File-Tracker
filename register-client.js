(function registerModules() {
  ...
  async function openDscForm(existing = null) {
    const [clientResult, boxResult] = await Promise.all([api("/api/clients/search?limit=50"), api("/api/dsc/boxes")]); ui.clients = clientResult.clients || []; ui.boxes = boxResult.boxes || [];
    showModal(existing ? "Edit DSC" : "Add DSC to Master", `<form id="dscForm" class="register-form-grid register-form-wide">
      <label>Client Master<select name="clientId" id="dscClient"><option value="">Select client</option>${ui.clients.map((c) => `<option value="${c.id}" data-name="${esc(c.client_name)}" data-pan="${esc(c.pan_reg_no || "")}" data-phone="${esc(c.contact_number || "")}" data-email="${esc(c.email || "")}">${esc(c.client_name)}${c.pan_reg_no ? ` — ${esc(c.pan_reg_no)}` : ""}</option>`).join("")}</select></label>
      ${inputField("Client Name","clientName","",true)}${inputField("PAN","pan")}${inputField("Entity Name","entityName")}${inputField("DSC Holder Name","holderName","",true)}${inputField("Holder Designation","holderDesignation")}${inputField("DIN","din")}${inputField("Mobile","mobile","","","tel")}${inputField("Email","email","","","email")}
      ${inputField("DSC Type","dscType")}${inputField("Class / Certificate Type","certificateClass")}${selectField("Individual / Organisation","holderType",["Individual","Organisation"])}${inputField("Token Make","tokenMake")}${inputField("Token Serial No.","tokenSerial","",true)}${inputField("Certificate Serial No.","certificateSerial")}
      ${inputField("Issued Date","issuedDate","","","date")}${inputField("Valid From","validFrom","","","date")}${inputField("Expiry Date","expiryDate","","","date")}${selectField("Status","status",["Fresh Issue Pending","Application in Progress","Verification Pending","Issued","Active","Expiring Soon","Renewal Initiated","Renewal in Progress","Renewed","Expired","In Office","Issued Out","Returned","Lost / Missing","Damaged","Revoked","Closed"],"In Office")}
      <label>Current Box<select name="boxId"><option value="">Unboxed</option>${ui.boxes.map((b) => `<option value="${b.id}">${esc(b.box_code)} — ${esc(b.location)}</option>`).join("")}</select></label>${inputField("Slot / Position","slotPosition")}
      <label>Assigned Staff<select name="assignedUserId"><option value="">Not assigned</option>${ui.users.map((u) => `<option value="${u.id}">${esc(u.name)}</option>`).join("")}</select></label><label class="form-span-2">Remarks — never enter PIN/password<textarea name="remarks" rows="3"></textarea></label>
      <div class="security-callout form-span-2">DSC PINs and passwords are intentionally not stored, displayed, exported or placed in QR codes.</div><div class="modal-actions form-span-2"><button type="button" class="secondary-button" data-close-register-modal>Cancel</button><button type="submit" class="primary-button">Save DSC</button></div></form>`);
    const form = document.querySelector("#dscForm"); form.querySelector("#dscClient").onchange = (event) => { const o=event.target.selectedOptions[0]; if(!o?.value)return; form.elements.clientName.value=o.dataset.name; form.elements.pan.value=o.dataset.pan; form.elements.mobile.value=o.dataset.phone; form.elements.email.value=o.dataset.email; };
    form.onsubmit = async (event) => { event.preventDefault(); await submitJson("/api/dsc","POST",formObject(form)); closeModal(); toast("DSC added to Master."); window.renderDscRegisterPage(); };
  }

  async function openDscDetail(id) {
    const result = await api(`/api/dsc/${id}`); const r=result.record;
    showModal(`${r.dsc_id} — ${r.holder_name}`, `<div class="register-detail"><div class="detail-summary-grid"><span><small>Client</small><strong>${esc(r.client_name)}</strong></span><span><small>Token Serial</small><strong>${esc(r.token_serial)}</strong></span><span><small>Expiry</small><strong>${date(r.expiry_date)}</strong></span><span><small>Status</small>${badge(r.status)}</span><span><small>Custody</small><strong>${esc(r.current_custody)}</strong></span><span><small>Storage</small><strong>Box: ${esc(r.box?.box_code || "—")} | Slot: ${esc(r.slot_position || "—")}</strong></span></div>
  ...
  async function viewBox(id) { const result=await api(`/api/dsc/boxes/${id}`); showModal(`Box ${result.box.box_code}`, `<div class="detail-summary-grid"><span><small>Location</small><strong>${esc(result.box.location)}</strong></span><span><small>Capacity</small><strong>${result.box.capacity}</strong></span><span><small>Occupied</small><strong>${result.box.occupied}</strong></span><span><small>Available</small><strong>${result.box.available}</strong></span></div>${genericRows(result.records,["slot_position","client_name","holder_name","token_serial","expiry_date","status"])}<div class="modal-actions"><button class="secondary-button" onclick="window.print()">Print Box List</button></div>`); }
